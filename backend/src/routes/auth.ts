...
router.post("/login", async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    const token = jwt.sign(
      { id: user._id, email: user.email, isAdmin: user.isAdmin },  // Added isAdmin field
      JWT_SECRET, 
      { expiresIn: "1d" }
    );
...
// ==========================
router.get("/admin", protect, adminOnly, async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
